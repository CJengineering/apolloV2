...
export default function SocialMediaList({ socialMediaLinks }: { socialMediaLinks: SocialMediaLinks }) {
  const renderLink = (name: string, url: string) => {
    const iconPath = socialIcons[name.charAt(0).toUpperCase() + name.slice(1)];
    if (!iconPath) return null;

    return (
      <a key={name} className="pr-2" href={url} target="_blank" aria-label={name.charAt(0).toUpperCase() + name.slice(1)}>
  ...
  return (
    <div className="flex">
      {socialMediaLinks.instagram && renderLink(socialMediaLinks.instagram.name, socialMediaLinks.instagram.url)}
      {socialMediaLinks.youtube && renderLink(socialMediaLinks.youtube.name, socialMediaLinks.youtube.url)}
      {socialMediaLinks.facebook && renderLink(socialMediaLinks.facebook.name, socialMediaLinks.facebook.url)}
      {socialMediaLinks.linkedin && renderLink(socialMediaLinks.linkedin.name, socialMediaLinks.linkedin.url)}
      {socialMediaLinks.twitter && renderLink(socialMediaLinks.twitter.name, socialMediaLinks.twitter.url)}
