...
const FilterComponent: React.FC = () => {
  const { programmes, sources, setProgrammeFilter, setSourceFilter } = useNewsContext();
  ...
  const filteredProgrammes = queryProgramme === '' ? programmes : programmes.filter(programme => programme.name.toLowerCase().startsWith(queryProgramme.toLowerCase()));
  const filteredSources = querySource === '' ? sources : sources.filter(source => source.name.toLowerCase().startsWith(querySource.toLowerCase()));
  ...
  return (
    <div className="w-full">
      <div className="flex space-x-4 mb-4">
        <div className="w-1/2">
          <Combobox value={queryProgramme} onChange={(value) => setQueryProgramme(String(value))}>
            {({ open }) => (
              <>
                <Combobox.Label className="block text-sm font-medium text-gray-700">Select Programme</Combobox.Label>
                <div className="mt-1 relative">
                  <div className="relative w-full text-left bg-white rounded-lg cursor-default overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm">
                    <Combobox.Input
                      ...
                      onChange={(event) => setQueryProgramme(event.target.value)}
                      displayValue={(programme: RelatedCollection) => programme.name}
                    />
                  ...
                  >
                    <Combobox.Options className="absolute z-40 mt-1 w-full bg-white max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
                      {filteredProgrammes.length === 0 && queryProgramme !== '' ? (
                        ...
                        filteredProgrammes.map((programme) => (
                          <Combobox.Option
                            ...
                            className={({ active }) => `cursor-default select-none relative py-2 pl-3 pr-9 ${active ? 'text-white bg-indigo-600' : 'text-gray-900'}`}
                          >
                            {({ selected, active }) => (
                              <>
        ...
        <div className="w-1/2">
          <Combobox value={querySource} onChange={(value) => setQuerySource(String(value))}>
            {({ open }) => (
              <>
                <Combobox.Label className="block text-sm font-medium text-gray-700">Select Source</Combobox.Label>
                <div className="mt-1 relative">
                  <div className="relative w-full text-left bg-white rounded-lg cursor-default overflow-hidden focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:text-sm">
                    <Combobox.Input
                      className="w-full border border-gray-300 rounded-md pl-3 pr-10 py-2 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {selectedProgrammes.map((programme) => (
            <div key={programme.id} className="flex items-center justify-between p-2 bg-blue-200 rounded-md">
              ...
              <button onClick={() => handleRemove(programme.id, 'programme')} className="ml-4 text-red-600 hover:text-red-800 focus:outline-none">
          ...
          {selectedSources.map((source) => (
            <div key={source.id} className="flex items-center justify-between p-2 bg-green-200 rounded-md">
              ...
              <button onClick={() => handleRemove(source.id, 'source')} className="ml-4 text-red-600 hover:text-red-800 focus:outline-none">
                <XMarkIcon className="h-5 w-5" />
              </button>
            </div>
          ))}
